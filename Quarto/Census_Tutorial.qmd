...
The geography aggregation level available differs between get_acs() and get_decennial() with the latter going down to the Census block-level and the former having the small enumeration unit of block group. Other political and statistical enumeration levels are available as well, see section 2.2 in book for table.
